...
def do_test_harmonic_sequence(N, expected):
    ...
    actual = se6.harmonic_sequence(N)
    check_none(actual, recreate_msg)
    check_type(actual, expected, recreate_msg)
    check_float_equals(actual, expected, recreate_msg)

def do_test_recursive_len(lst, expected):
    recreate_msg = gen_recreate_msg(MODULE, "recursive_len", lst)
    actual = se6.recursive_len(lst)
    check_none(actual, recreate_msg)
    check_type(actual, expected, recreate_msg)
    check_equals(actual, expected, recreate_msg)
...
def test_recursive_len_1():
    lst = []
    do_test_recursive_len(lst, 0)

def test_recursive_len_2():
    lst = [1]
    do_test_recursive_len(lst, 1)

def test_recursive_len_3():
    lst = ["hi"]
    do_test_recursive_len(lst, 1)

def test_recursive_len_4():
    lst = [1, 2]
    do_test_recursive_len(lst, 2)

def test_recursive_len_5():
    lst = ["hi", 5]
    do_test_recursive_len(lst, 2)

def test_recursive_len_6():
    lst = [1, 2, 3, 4, 5, 6]
    do_test_recursive_len(lst, 6)

def test_recursive_len_7():
    lst = [0, 1, 2, 3, 4, 5, 6]
    do_test_recursive_len(lst, 7)

def test_recursive_len_8():
    lst = list(range(0, 100))
    do_test_recursive_len(lst, 100)
